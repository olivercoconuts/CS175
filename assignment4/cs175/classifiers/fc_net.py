...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out, cache_hidden = affine_relu_forward(X, W1, b1)
        scores, cache_scores = affine_forward(out, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dout, dW2, db2 = affine_backward(dscores, cache_scores)
        dW2 += self.reg * W2

        dx, dW1, db1 = affine_relu_backward(dout, cache_hidden)
        dW1 += self.reg * W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, hd in enumerate(hidden_dims):
            self.params['W%d' % (i + 1)] = np.random.randn(prev_layer_dim, hd) * weight_scale
            self.params['b%d' % (i + 1)] = np.zeros(hd) * weight_scale
            prev_layer_dim = hd
        self.params['W%d' % (self.num_layers)] = np.random.randn(prev_layer_dim, num_classes) * weight_scale
